...
eval_dataloader = DataLoader(data, batch_size=3, shuffle=False)
...
for images, questions, img_ids in tqdm(eval_dataloader):
    ...
    for it in range(ITER):
        embed = model.encode_img(images + delta)[0]
        with torch.no_grad():
            target_embed = model.encode_img(images)[0]
        loss = criterion(embed, target_embed)        
        model.zero_grad()
